	...
	while (strcmp(opt, "F")) {
		...
		try {
			...
			if (!strcmp(opt, "G"))
			{
				...
				//f(x)=mx+b
				long double serieT1, serieT2;
				long double serieV1, serieV2;
				int sizeT = round(tiempo.size() / 2);

				for (int i = 0; i < tiempo.size(); i++) {
					if (i < sizeT) {
						serieT1 = serieT1 + tiempo[i];
						serieV1 = serieV1 + volumenrelativo[i];
					}
					else {
						serieT2 = serieT2 + tiempo[i];
						serieV2 = serieV2 + volumenrelativo[i];
					}
				}

				serieT1 = serieT1 / sizeT;
				serieT2 = serieT2 / sizeT;
				serieV1 = serieV1 / sizeT;
				serieV2 = serieV2 / sizeT;

				long double mx = ((serieV2 - serieV1) / (serieT2 - serieT1));
				long double b = (mx * serieT1) + serieV1;
				
				cout << "serieT1: " << serieT1 << " serieT2: " << serieT2 << " serieV1: " << serieV1 << " serieV2: " << serieV2 << endl;
				cout << "f(x) = " << mx << " *x + " << b << endl;

				/*std::vector<long double> pendiente;

				for (int i = 0; i <= tiempo.size(); i++) {
					pendiente.push_back(mx*tiempo[i]+b);
				}*/
